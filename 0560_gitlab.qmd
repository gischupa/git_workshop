...
* Diese Schritte müssen alle als *root* erfolgen, 
  also wechsle die Rolle mit \cmd{sudo -i}. Das gilt 
  jetzt bis zur Abmeldung.
* Suche in der Datei \datei{/etc/ssh/sshd\_config} nach der 
  Zeile **# Port 22** (Auskommentiert). Das geht gut mit \cmd{nano}:  
...
Die aktuelle Session läuft weiter (Port 22), neue 
Verbindungen müssen ab jetzt über Port 23 laufen:
...
```bash 
cd ~/dilligen_material_git/material/docker/gitLab 
```

Dort findest du die \datei{docker-compose.yml}. In dieser 
Datei findest eine IP-Adresse. Hier im Kurs unbedingt die IP-Adresse 
vom Ubuntu stehen. Im Praxiseinsatz würdest du den Domain-Name 
deines \Gitlab verwenden und auf https wechseln.

**WICHTIG**  
Da hier im Kurs bei jedem Reboot die IP-Adresse wechselt,
müssen wir ab jetzt unbedingt Reboots vermeiden!!!!
...
services:
  web:
    ...
    restart: always
    hostname: gitlab  # Im Netz 'gitlab.server.de'
    environment:
      GITLAB_OMNIBUS_CONFIG: |
        external_url 'http://192.168.3.190:443' # im Netz https
        gitlab_rails['gitlab_shell_ssh_port'] = 22
    ports:
      - '22:22'  # 23:22 falls 22 auf 23 umgestellt
      - '80:80'
      - '443:443'
    volumes:
      - ./config:/etc/gitlab
...
da es *external* ist, aber das kennst du bereits von \gitea:
...
stark vom verfügbaren Hauptspeicher ab. Mit 4GB bin ich an die 
...
Weboberfläche möglich. Hier solltest Du nun auch sofort 
das Kennwort ändern (Wieder ist das Erstkennwort erforderlich!).
...
Bildern ein geringeres ökologisches Problem!
