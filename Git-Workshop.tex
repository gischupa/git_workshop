...
vorgenommen hat\footnote{Wir sprechen hier NICHT von einer Absicherung
  gegen Dummheit oder Bosheit. Sobald Änderungen von papst@vatikan.va
  auftauchen, wäre ich skeptisch!}.

\samplestart

\textbf{TIPP}\\
Im Terminal kann mit der \emph{Pfeil hoch} Taste durch die letzten
Befehle geblättert werden -- damit gehen manche Dinge sehr schnell!
\sampleend
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{git}\NormalTok{ init hands\_on\_1 }
\BuiltInTok{cd}\NormalTok{ hands\_on\_1}
\end{Highlighting}
\end{Shaded}

Alternativ kannst du das auch mit mehreren Schritten erledigen:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mkdir}\NormalTok{ hands\_on\_1 }
\BuiltInTok{cd}\NormalTok{ hands\_on\_1 }
\FunctionTok{git}\NormalTok{ init }
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\ExtensionTok{Hinweis:}\NormalTok{ Als Name für den initialen Branch wurde }\StringTok{\textquotesingle{}master\textquotesingle{}}\NormalTok{ benutzt. }
\ExtensionTok{Hinweis:}\NormalTok{ Dieser Standard{-}Branchname kann sich ändern.}
\ExtensionTok{Hinweis:}\NormalTok{ Um den Namen des initialen Branches zu konfigurieren,}
\ExtensionTok{Hinweis:}\NormalTok{ der in allen neuen Repositories verwendet werden soll und}
\ExtensionTok{Hinweis:}\NormalTok{ um diese Warnung zu unterdrücken, führen Sie aus:}
\ExtensionTok{Hinweis:} 
\ExtensionTok{Hinweis:}\NormalTok{    git config }\AttributeTok{{-}{-}global}\NormalTok{ init.defaultBranch }\OperatorTok{\textless{}}\NormalTok{Name}\OperatorTok{\textgreater{}}
\ExtensionTok{Hinweis:} 
...
\ExtensionTok{Hinweis:} 
\ExtensionTok{Hinweis:}\NormalTok{    git branch }\AttributeTok{{-}m} \OperatorTok{\textless{}}\NormalTok{Name}\OperatorTok{\textgreater{}}
\ExtensionTok{Leeres}\NormalTok{ Git{-}Repository in /tmp/hands\_on\_1/.git/ initialisiert}
\end{Highlighting}
\end{Shaded}
...
\textbf{Hinweis}\\
Du kannst \git auch so konfigurieren, dass der Branch eines neu
angelegten Repositories automatisch \emph{main} genannt wird.
...
Durch die Kombination aus \emph{add} und \emph{commit} hast du einen
\emph{Meilenstein} in der Projekthistorie erstellt. Damit kennst bereits
einen typischen Ablauf von \git:

Du sammelst Dateien mit \emph{add}, in denen du Änderungen vorgenommen
hat und schreibst diese Änderungen mit \emph{commit} in die
Projekthistorie. Wie oft, mit wie vielen Dateien in welchen
Zeitabständen \ldots{} das ist ziemlich alleine \emph{deine}
Entscheidung.
...
Die Historie besitzt nun zwei Punkte, an die du zurückgekehren könntest
-- wie das geht, lernst du noch.

Natürlich ist das \emph{adden} nicht auf eine einzige Datei beschränkt.
Liegen mehrere geänderte Dateien vor, so müssen sie nicht einzeln
hinzugefügt werden. Es gibt dafür einen \emph{Sammelbefehl}, der den
gesamten Ordner mit allen Unterordnern hinzufügt:
...
diese Dateien dann im gleichen Commit festgeschrieben werden. In einem
solchen Fall solltest du bei \emph{add} genau die Dateinamen angeben.
...
\textbf{Hinweis}\\
Es gibt sogar die Möglichkeit, dass man nur Teile der Änderungen in
einer Datei in einen Commit und die restlichen Änderungen in einen
anderen Commit übernimmt. Das sind aber sehr spezielle Varianten, die
wir nicht behandeln! \sampleend
...
\emph{logischen} Bereiche sprechen:
...
\item
  Repository\footnote{Je nach Quelle und Kontext etwas inkonsistent
    verwendet}
\end{itemize}
...
Du bist immer noch im aktuellen Ordner \ordner{hands\_on\_1}. Er sollte
...
\textbf{Hinweis}\\
In Bezug auf \emph{add} muss man zwei Dinge im Hinterkopf behalten. 1.
Die Abkürzung \cmd{git commit -a} führt zwar im Hintergrund ein
\emph{add} aus, aber nur für bereits versionierte Dateien --
\datei{datei2.txt} würde also ignoeriert.\\
2. Der Befehl \cmd{git add .} findet nur Dateien im aktuellen Ordner und
den Unterordnern. Sollen auch Dateien in übergeordneten oder
\emph{parallelen} Ordnern berücksichtigt werden, so brauchst du
\cmd{git add --all}. \sampleend
...
die Projekthistorie -- führst du aber \textbf{nicht aus}!.
...
in die Projekthistorie. Das führt dann zu folgender Situation:
...
aktuellen Änderungen nach wie vor im Arbeitsverzeichnis vorhanden sind,
weil für sie noch kein \emph{add} stattgefunden hat.
...
\textbf{commit}\\
Zeigt auf den Tree, der zum Zeitpunkt des Commits aktuell war. Er
enthält noch weitere Metadaten.

\textbf{tag}\\
Zusatznamen für ausgewählte Commits (Releases)

\textbf{Sehen wir uns das genauer an}
...
Lege nun im leeren \work eine Datei an\footnote{Ich habe absichtlich
...
Du siehst erste Änderungen im \emph{.git}-Ordner. Im Bild siehst du aus
Platzgründen nur den relevanten Ausschnitt. Bei dir kann der Hash einen
...
\ExtensionTok{│  }\NormalTok{ └── exclude}
\ExtensionTok{├──}\NormalTok{ objects}
\ExtensionTok{│  }\NormalTok{ ├── ce          }\OperatorTok{\textless{}\textless{}\textless{}}\NormalTok{ neu, bei dir evtl anders! }
...
Mit \texttt{git\ log} erfährst du etwas über den erfolgten Commit -- zum
...
Dateien zwischen zwei Commits nicht (Hash bleibt gleich), so werden sie
nicht neu gespeichert. Der neue Commit zeigt auf einen neuen Tree in dem
wieder die gleichen Hashes mit den gleichen Dateinamen stehen.
...
Der Fehler ist ganz frisch \ldots{}
...
\CommentTok{\#\#\#\#\#\# Alte Version wieder herstellen}
...
\CommentTok{\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#}
\CommentTok{\#\#\#\#\#\# Das geht jetzt nicht, weil Stage = Workdir}
\CommentTok{\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#}
\FunctionTok{git}\NormalTok{ restore datei.txt  }
\end{Highlighting}
\end{Shaded}

Du musst nun mit \cmd{git log} den Hash vom letzten Commit ermitteln und
dann geht
...
Sind viele Dateien \emph{verkorkst}, so bietet sich ein komplettes
Rücksetzen auf einen alten Stand an. Allerdings gibt es da einige
Varianten (siehe Tabelle unten).

Wird der Befehl ohne \cmd{--hard} ausgeführt, so wird nur der Stage auf
den Stand des alten Commit gesetzt, nicht aber das \work. Aber
eigentlich verlassen wir hier schon die einfachen Fälle. Ich persönlich
verliere bei so vielen Varianten bald den Überblick und von einem
\emph{routinierten Handling} kann keine Rede sein!
...
\bottomrule\noalign{}
\endlastfoot
\texttt{git\ reset\ -\/-hard\ HEAD} & Setzt Arbeitsverzeichnis und
Staging auf \texttt{HEAD} zurück; alle uncommitted Änderungen werden
gelöscht. & Unwiderruflich, wenn Änderungen nicht anderweitig gesichert
sind. \\
\texttt{git\ reset\ -\/-hard\ HEAD\textasciitilde{}1} & Branch-Zeiger
bewegt sich ein Commit zurück; Arbeitsverzeichnis entspricht dem
vorletzten Commit. & Vorsichtig bei bereits gepushten Commits --- Remote
bleibt davon unberührt. \\
\texttt{git\ reset\ -\/-soft\ HEAD\textasciitilde{}1} & Entfernt letzten
Commit, macht die Änderungen als gestaged verfügbar (Index enthält sie).
& Gut zum Kombinieren/Neuformulieren von Commits. \\
\texttt{git\ reset\ -\/-mixed\ HEAD\textasciitilde{}1} (oder
\texttt{git\ reset\ HEAD\textasciitilde{}1}) & Entfernt Commit;
Änderungen bleiben im Working Directory, aber sind nicht gestaged. &
Standardmodus (\texttt{-\/-mixed}) wenn kein Flag angegeben. \\
\texttt{git\ reset\ HEAD} & Entfernt Dateien aus dem Index; Änderungen
bleiben im Arbeitsverzeichnis. & Nützlich, um versehentliches
\texttt{git\ add} rückgängig zu machen. \\
\texttt{git\ reset\ -\/-hard\ \textless{}commit-hash\textgreater{}} &
Setzt Branch auf den angegebenen Commit; Arbeitsverzeichnis wird
entsprechend angepasst. & \texttt{\textless{}commit-hash\textgreater{}}
vorher mit \texttt{git\ log}/\texttt{git\ rev-parse} prüfen. \\
\texttt{git\ reset\ -\/-hard\ HEAD\textasciitilde{}1} +
\texttt{git\ push\ -\/-force} & Lokaler und Remote-Branch zeigen auf den
älteren Commit (push überschreibt Remote). & \textbf{Gefährlich}: ändert
history auf Remote --- Absprache mit Team erforderlich. \\
\end{longtable}
...
\ExtensionTok{Sie}\NormalTok{ befinden sich im Zustand eines }\StringTok{\textquotesingle{}losgelösten HEAD\textquotesingle{}}\NormalTok{. Sie können sich}
\ExtensionTok{umschauen,}\NormalTok{ experimentelle Änderungen vornehmen und diese committen, und}
\ExtensionTok{Sie}\NormalTok{ können alle möglichen Commits, die Sie in diesem Zustand machen,}
\ExtensionTok{ohne}\NormalTok{ Auswirkungen auf irgendeinen Branch verwerfen, indem Sie zu einem}
\ExtensionTok{anderen}\NormalTok{ Branch wechseln.}

\ExtensionTok{Wenn}\NormalTok{ Sie einen neuen Branch erstellen möchten, um Ihre erstellten Commits}
\ExtensionTok{zu}\NormalTok{ behalten, können Sie das }\ErrorTok{(}\ExtensionTok{jetzt}\NormalTok{ oder später}\KeywordTok{)} \ExtensionTok{durch}\NormalTok{ Nutzung von}
\StringTok{\textquotesingle{}switch\textquotesingle{}}\NormalTok{ mit der Option }\AttributeTok{{-}c}\NormalTok{ tun. Beispiel:}
...
\ExtensionTok{Sie}\NormalTok{ können diesen Hinweis ausschalten, indem Sie die Konfigurationsvariable}
\StringTok{\textquotesingle{}advice.detachedHead\textquotesingle{}}\NormalTok{ auf }\StringTok{\textquotesingle{}false\textquotesingle{}}\NormalTok{ setzen.}

\ExtensionTok{HEAD}\NormalTok{ ist jetzt bei 95a37c9 Version 7}
\end{Highlighting}
\end{Shaded}

\subsection{Es ging nur wenig schief}\label{es-ging-nur-wenig-schief}

Du hast nach dem letzten Commit an einer Datei weiter gearbeitet, bist
mit dem Ergebnis aber nicht zufrieden. Du möchtest die letzte Version
der Datei wieder herstellen:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{git}\NormalTok{ init sz1}
\BuiltInTok{cd}\NormalTok{ sz1}
\FunctionTok{git}\NormalTok{ branch }\AttributeTok{{-}m}\NormalTok{ main}
...
\texttt{git\ restore\ \textless{}dateiname\textgreater{}} wieder mit der
Version aus dem Stage überschrieben werden.
...
Dass der Stage wirklich nicht leer ist, zeigt folgendes Beispiel, das
eine Reparatur am \work zeigt.
...
\BuiltInTok{echo} \StringTok{"falscher Text"} \OperatorTok{\textgreater{}\textgreater{}}\NormalTok{ datei.txt   }\CommentTok{\#  Fehler im Arbeitsverzeichnis }
\FunctionTok{git}\NormalTok{ restore datei.txt              }\CommentTok{\# repariert mit Datei aus Stage}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  den Stage geschoben wurde. Dabei darf das \work nicht verändert
  werden. Das erreichst du mit
...
\phantomsection\label{annotated-cell-190}%
\begin{Shaded}
\begin{Highlighting}[]
...
\phantomsection\label{annotated-cell-195}%
\begin{Shaded}
\begin{Highlighting}[]
\AttributeTok{  }\FunctionTok{runner}\KeywordTok{:}
