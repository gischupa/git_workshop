...
git tag -a Zentraldokument -m "Zentraldokument"

##### Schülerin erstellt branch
git switch -c susanne
...
git tag -a Susanne1 -m "Geschichte begonnen"
...
git tag -a Anleitung_II -m "Anleitung_II"

########### Susanne schreibt weiter - branch 1
git switch susanne
git switch -c "umkehr"
cat <<EOF>> gini.md
Hänsel überlegte einige Zeit, was er nun machen sollte. In dieses Haus gehen
und nach dem seltsamen *Gini* fragen, oder umkehren und so tun, als sei dieser
...
Er schlich sich die Treppe hinauf und lauschte an der Türe.
