...
3. Die Daten von Gitea liegen im Ordner \datei{gitea\_data},
   genauer unter \datei{/var/lib/docker/volumes/gitea\_data}.
...
4. Ein Volume kann auch nur eine einzige Datei sein. Beachte
   den Unterschied bei der Schreibung von \datei{./config.yaml} und
   \datei{gitea\_runner\_data}. Das *./* am Anfang bedeutet, dass diese 
   Datei im gleichen Ordner liegt wie die \datei{docker-compose.yaml}.
...
anlegen* (o.ä.) klicken. Dort erscheint dann das Token, das in 
die \datei{docker-compose.yaml} eingetragen werden muss. 
Beim Neustart registriert sich der Runner 
...
du den Gitea- und den DB-Server mit \cmd{ctrl-c} zunächst wieder beenden.
Das Token wird nun in die \datei{docker-compose.yaml} kopiert.
...
Hat das ohne Fehler geklappt, dann wieder \key{strg + c} und dann
ein richtiger Start:
