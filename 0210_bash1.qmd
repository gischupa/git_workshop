# Hands On I
Bevor wir beginnen, müssen einmalig einige Konfigurationen vorgenommen werden -- einige davon betreffen auch bereits den späteren Einsatz im 
Netz, wo klar sein muss, welcher Benutzer Änderungen an den Dateien vorgenommen hat^[Wir sprechen hier NICHT von einer Absicherung gegen Dummheit oder Bosheit. Sobald Änderungen von papst@vatikan.va auftauchen, wäre ich skeptisch!].
...
Öffne die \git-Bash^[bzw. das Terminal deines Systems. Das setze ich in Zukunft stillschweigend voraus]. Da es sich um
globale Einstellungen handelt, muss das nicht unbedingt im Projektordner sein!^[Sollte dir 
