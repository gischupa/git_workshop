...
# Evtl. direkt auf der Seite https://dl.gitea.com/gitea die 
# aktuelle Version checken  und hier anpassen.
wget -O gitea \
...
#
###
# (You can also provide gitea an http 
# fallback and/or ssh socket too)
#
...
Dazu muss in der Datei \datei{/etc/gitea/app.ini} die folgende Zeile 
...
Melden Sie sich als Administrator in Gitea an und rufen Sie das 
...
System erlauben. Stellen Sie im aufklappbaren Detail-Abschnitt ein, 
...
das man mit einer Namensdatei als Parameter aufrufen kann, 
...
user=susi
curl -X PUT \
"http://192.168.3.195:3000/api/v1/orgs/${name}$/members/${user}" \
-H "Authorization: token ${token}"
```
