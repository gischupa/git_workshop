...
im *main*-Branch zusammengeführt werden. Hier soll aber  nicht \git
...
*package* ist ein relativ großer LiveCycle, der
...
ich will schließlich TDD demonstrieren und keinen Programmierkurs 
...
Test für eine *noch nicht existierende* Funktion. Bei Hard-Core TDD würdest 
...
public void cryptLiefertEineRueckgabe(){
    Caesar caesar = new Caesar();  // Objekt erstellen
    String res = caesar.crypt();   // Das gibt es noch nicht
    assertNotNull(res);            // Greift, wenn es klappt
}
```

Der Befehl \cmd{mvn test} scheitert, wegen der fehlenden Methode 
bereits beim Compilieren.
Du schreibst zunächst folgende, minimal funktionierende, Variante der Methode:
...
Der Test geht natürlich immer noch schief, denn es wird ja auf `NotNull` getestet.
Dieses Versagen des Tests ist aber wichtig, denn 
...
@Test
    public void cryptLiefertLeerWennKeinKlartext(){
        ...
        assertEquals(0, res.length()); // Scheitert wegen "x"
    }
```
...
Rot-Phase ... Minimal funktionierende Funktion schreiben
...
kleine Verschiebungen in der Mitte des Alphabets.
...
Wir könnten nun auch noch die Verschiebungen in 
...
Hier ist da bei mir dann auch das Ende erreicht, weil ich von diesem Weg 
