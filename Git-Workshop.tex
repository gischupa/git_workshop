...
Wir werden mit der Installation der Software beginnen und zunächst lokal
auf dem Computer die Grundlagen erarbeiten. Im Anschluss kümmern wir und
um Aspekte der kooperativen Arbeit in Netzwerken. Die Installation eines
eigenen \git-Server bildet dann den Abschluss des ersten Teils.

Im zweiten Teil soll es dann um den Praxiseinsatz im Umfeld von SDLC und
CI/CD gehen.

Für den reinen Client-Aspekt ist die Verwendung von \git auf Windows
problemlos, sobald es aber um Server geht, sind bessere Systeme wie
Linux oder MacOS sinnvoller\footnote{Ich habe bisher noch keinen Server
  im Internet getroffen, auf dem MacOS als Sytem verwendet wird. Das
...
\item
  Installation VB
\item
  Deb-Distro \ldots{} wahrscheinlich Ubuntu
\item
...
\section{Schülerverwaltung (draft)}\label{schuxfclerverwaltung-draft}
...
\item
  SCP einer Schülerliste + Anschluss-Script? Problem ist hier die
  ``Verwaltung'' der Klassen und User. Ein ncurses-script könnte da
  hilfreich sein, nach ssh-login
\item
  Geht das evtl auch über einen ``Zwergen''\_Web-server, der aus dem
  Terminal gestartet wird?
\item
  Git-Server für jede Klasse mit Docker sinnvoll?
\item
  Sollen die Schüler einen personalisierten Login haben oder als
  User1-20 laufen?
\item
  Sollen sich die Schüler anmelden dürfen oder nur PK für git?
\end{itemize}
...
\tightlist
\item
  die einzelnen Instanzen können nicht alle Port 22 verwenden! den 22er
  brauchen wir wohl für den HOST
\item
...
\section{Notizen}\label{notizen}

ssh evtl NICHT als root, sondern als normaler User, dann muss aber sudo
eingerichtet werden. das geht schon wieder TIEF ins Wissen \ldots{}
...
Da \git als Serveranwendung bekannt ist, stellt sich die Frage, warum
man auf seinem eigenen Rechner ein Versions-Kontroll-System beim
Erstellen von Projekten einsetzen sollte.\\
Neben dem Aspekt, dass bestimmte Probleme beim Backup so gelöst
werden\footnote{Natürlich ist ein Backup auf dem gleichen Rechner
  relativ sinnlos. Es spricht aber nichts dagegen, mit \git auf einer
  externen Festplatte zu arbeiten!}, hat man den Vorteil, ungefährdet
mit verschiedenen Versionen seines Projekts zu experimentieren, ohne
einen Totalverlust befürchten zu müssen. Im Vorgriff sei gesagt, dass
\git im Prinzip nur einen eigenen Ordner für das Projet benötigt und
hier ist es relativ egal, wo sich dieser Ordner befindet!

\section{Installation}\label{installation-1}

Beginnen wir mit Linux, weil dort der Aufwand am geringsten ist. Hier
genügt es bei einer Debian-basierenden Version, wenn man im Terminal den
Befehl eingibt:
...
auf anderen Rechner kann es bereits installiert sein, falls Xcode
installiert ist. Im Zweifelsfall gibt es aber auch einen Installer unter

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{https://git{-}scm.com/download/mac}
\end{Highlighting}
\end{Shaded}

Auch für Windows ist auf dieser Seite ein Installer verfügbar, oder man
verwendet den Appstore\footnote{Da ich mit Software-Versionen aus dem
  App-Store schon öfter auf die Nase gefallen bin, bevorzuge ich den
  anderen Weg!}. Ein detaillierter Ablauf einer Installation befindet
sich im Anhang. Leider ist es mir nicht möglich, minimal Änderungen an
den Screenshots ständig aktuell zu halten.

TODO: Installationsanleitung GIT auf Windows in den Anhang schieben.

\section{Erster Kontakt}\label{erster-kontakt}

Wir verzichten am Anfang konsequent auf den Einsatz grafischer
Hilfsprogramme. Auf diese Weise haben wir auf allen drei
Betriebssystemen weitgehend die gleichen Vorgehensweisen und wird lernen
wirklich die Grundlagen.

Im ersten Schritt muss hierzu ein Terminal geöffent werden. Auf Linux
und MacOS ist das auch wirklich unter diesem Namen zu finden, bei
Windows nennt sich das Programm \git-Bash. Natürlich brauchen wir auch
einen Projektordner für unsere sämtlichen Experimente. Ich setze voraus,
dass jeder für sein System in der Lage ist, einen Ordner mit dem Namen
\emph{git\_projekte} zu erstellen. Ziel wäre es im Anschluss, dass wir
uns im Terminal in diesem Ordner befinden.

Bei Linux und MacOS erreichen wir das direkt im Terminal mit folgenden
Befehlen (jweils mit \key{enter} abschließen):

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mkdir}\NormalTok{ git\_projekte }
\BuiltInTok{cd}\NormalTok{ git\_projekte }
\end{Highlighting}
\end{Shaded}

Um den Ordner auch später einfach wiederfinden zu können, sollten wir
...
Da auch die \git-Bash eine Form von abgespecktem Linux darstellt, gelten
die nachfolgenden Befehle auf allen drei Systemen in gleicher Weise. Es
sinnvoll, sich diese Liste bereit zu legen, oder die Befehle zu
verinnerlichen!
...
Für viele Beispiele benötigen wir schnell eine Datei mit minimalem
Inhalt, da es ja nur um Beispiele geht. Das geh im Prinzip mit dem
Editor von Windows, schneller ist es aber direkt im Terminal
...
Du Für das Grundverständnis ist es sinnvoll, zuerst mit dem reinen
...
\textbf{Hinweis}\\
Ab dieser Stelle wechsle ich vom neutralen \emph{man} auf das kollegiale
\emph{Du}, weil manche Formulierungen sonst unnötig klobig werden!
\sampleend

\section{Konfigurieren von git}\label{konfigurieren-von-git}

Öffne die \git-Bash\footnote{bzw. das Terminal deines Systems. Das setze
...
Zeitabständen \ldots{} das ist ziemlich alleine Deine Entscheidung.
...
festgeschrieben werden. In einem solchen Fall solltest du bei \emph{add}
...
Feinheiten und Varianten, von denen wir noch etliche in diesem Workshop
ansprechen werden.
...
uns hier nicht interessieren! \sampleend
...
Das Working-Directory enthält die Dateien an denen gearbeitet wird. Er
entspricht auf den ersten Blick genau einem normalen Ordner für die
Projektentwicklung. Erst durch die Initialisierung durch \git entsteht
ein weiterer, versteckter Ordner mit dem Namen \emph{.git}. Dieser
Ordner bildet den \emph{Stage} und das \emph{Repository}
gleichzeitig\footnote{Hier sieht man den inkonsistenten Gebrauch von
  „Repository``. Einerseits wird der gesamte Projektorder als Repository
  bezeichnet, sobald er von \git verwaltet wird, andererseits befindet
  sich das eigentliche Repository im Ordner \emph{.git}.}.
\emph{Logisch} bedeutet in diesem Zusammenhang, dass diese beiden
Bereiche von \git verwaltet werden und vom unbedarften Benutzer nicht
einfach unterschieden werden können. In einem späteren Abschnitt werden
wir einen genaueren Blick in diesen Ordner werfen.
...
die Projekthistorie. Der Befehl \cmd{git status} zeigt dir wieder den
aktuellen Zustand des \emph{Stage} an.
...
Du siehst erste Änderungen im \emph{.git}-Ordner. Ich gebe nur die
relevante Stelle an -- der Hashe lautet bei dir eventuell anders!
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{commit}\NormalTok{ 3b028e4d0230fb8f3553f0f7579e68e6c4e27d3f }\ErrorTok{(}\ExtensionTok{HEAD} \AttributeTok{{-}}\OperatorTok{\textgreater{}}\NormalTok{ main}\KeywordTok{)}
...
Der Hash 3b02\ldots{} scheint also ein Commit zu sein. Das kannst du mit
\cmd{git cat-file -t 3b028e4d0230fb8f3553f0f7579e68e6c4e27d3f} auch noch
...
\begin{Highlighting}[]
\CommentTok{\# Eingabe }
\FunctionTok{git}\NormalTok{ cat{-}file }\AttributeTok{{-}p}\NormalTok{ 3b028e4d0230fb8f3553f0f7579e68e6c4e27d3f}
...
Relativ unproblematisch sind \emph{Reparaturen} bei lokaler Arbeit, also
...
\phantomsection\label{annotated-cell-106}%
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{...}
\ExtensionTok{├──}\NormalTok{ config}
...
\section{Zu klärende Fragen}\label{zu-kluxe4rende-fragen-1}
...
\section{Nur git}\label{nur-git-1}
...
\subsection{Detaillierte Anleitung}\label{detaillierte-anleitung-1}
...
\section{Vorbereitung des Systems}\label{vorbereitung-des-systems-1}
...
\subsection{SSH-Verbindung}\label{ssh-verbindung-1}
...
\subsection{Software installieren}\label{software-installieren-1}
...
\subsection{Ordner für Repository}\label{ordner-fuxfcr-repository-1}
...
\phantomsection\label{annotated-cell-166}%
\begin{Shaded}
\begin{Highlighting}[]
...
\phantomsection\label{annotated-cell-171}%
\begin{Shaded}
\begin{Highlighting}[]
\AttributeTok{  }\FunctionTok{runner}\KeywordTok{:}
...
\section{Grundkonfiguration}\label{grundkonfiguration-2}
