...
Es handelt sich um ein öffentliches Repository, auf das auch ohne 
Benutzerkennung zugegriffen werden darf -- aber eben nur lesend. 
Aus diesem Grund wird zum Clonen auch *https* und nicht *ssh* verwendet.
Öffne die \gitbash (oder ein Terminal) und gib ein:


```bash
mkdir fortbildung
cd fortbildung
git clone https://TODO    storyvorlagen vorlage # Ziel: vorlage.git 
```

Es entsteht ein Ordner *storyvorlagen*, in dem die 
...
und dabei sind einzelne Fenster optimal (Wechsel z.B. mit \alt{tab}).
...
in der Anleitung entstehen würden. Leider ist das aber nicht möglich,
weil die Hashwerte nicht reproduzierbar sind -- zumindest auf 
meinem Rechner! Aus diesem Grund wurden im Vorlagenordner
...
Öffne im Fortbildungsordner eine \gitbash und erstelle
...
git init buch 
cd buch 
# oder mkdir buch; cd buch; git init
```   
...
Sie enthält (absichtlich) nur einen Teil der Geschichte! Das Kopieren kann auch 
...
5. In den nächsten drei Zeilen gab es keine Änderungen
6. Das ist vor der Änderung.
7. Diese Zeile steht in *b* -- wegen des „+“ Zeichens.
...
den Kern der Arbeit mit \git dar, zumindest wenn man alleine arbeitet.
...
Geschichten und Arbeitsversionen an unterschiedlichen 
...
Sie ist ebenfalls in dem Branch enthalten, du hast aber nicht vor,
...
Nun hast du das *Problem*, dass du drei Versionen von \datei{story2.md} hast:
...
* Im Buch-Ordner (aktueller Branch ist *entwicklung\_story\_2*)
* \cmd{git add story2.md}
* \cmd{git commit -m "typo 'weiße' korrigiert"}

Beachte, dass diese Änderung in den anderen beiden Branches *nicht* ankommt!
...
```bash
# Kontrolle ob Entwicklungszweig
git branch
# falls nötig
git switch entwicklung_story_2
```
...
```
Die Änderung ist also sichtbar. Der nachfolgende 
Befehl zeigt dir nichts, weil Commits verglichen werden!
...
Freddy war nicht allein. Seine beste Freundin,
Shelly, die >>>weise<<< ***weiße*** Schildkröte, lebte ebenfalls am
Teich. Shelly war für ihre ruhige und besonnene
Art bekannt, aber sie konnte auch sehr mutig sein,
wenn es darauf ankam.
@@ -31,3 +31,59 @@ Asphalt war mit Moos und Gras überwuchert. Freddy
...
Beachte, dass jetzt ein Fast-Forward-Merge nicht möglich ist, das 
...
Änderung durch. Verwende *suchen / ersetzen* oder im Terminal:
