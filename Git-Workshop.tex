...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{git}\NormalTok{ init }\AttributeTok{{-}{-}bare}\NormalTok{ minirepo.git}
...
\FunctionTok{git}\NormalTok{ clone }\VariableTok{$ARBEIT}\NormalTok{/minirepo susi }
\FunctionTok{git}\NormalTok{ clone }\VariableTok{$ARBEIT}\NormalTok{/minirepo max   }


\BuiltInTok{cd} \VariableTok{$SUSI} 
\BuiltInTok{echo} \StringTok{"Hallo"} \OperatorTok{\textgreater{}}\NormalTok{ datei.txt }
\FunctionTok{git}\NormalTok{ add datei.txt }
\FunctionTok{git}\NormalTok{ commit }\AttributeTok{{-}m} \StringTok{"Hallo geschrieben"}
...
und betrachte den Status erneut:
...
Repositories und sagt dir, dass dein \emph{Klon} einen Commit weiter
vorne ist, als das \emph{Original}.\\
Das macht \git, indem es \emph{Pointer} auf Commits setzt (also quasi
eine Datei im \texttt{.git}-Ordner führt, wo der aktuelle Hash enthalten
ist.)

\phantomsection\label{annotated-cell-89}%
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{...}
\ExtensionTok{├──}\NormalTok{ config}
...
\tightlist
\item[\circled{1}]
Enthält den Hash vom letzten Commit im Klon
\item[\circled{2}]
