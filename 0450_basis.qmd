...
(Nachfolgender Ablauf ist nur ein Vorschlag! Varianten stehen wieter unten.)  

Unter Linux / MacOS einfach eingeben:
```bash
ssh-keygen -t rsa -b 4096 -C "schueler@example.com"
```
Bei der Frage nach Speicherort und Dateiname nur *schule* angeben 
und die Fragen nach einer PassPhrase
beide Male mit der Enter-Taste übergehen.

Unter Windows geht das ab Windows 10 im Prinzip mit dem gleichen Befehl 
aus der *cmd.exe* oder *powershell* -- allerdings bin ich dabei auch
schon mit seltsamen Fehlern konfrontiert worden. Aus der git-Bash 
heraus sollte es aber eigentlich funktionieren.

Diesen Schritt muss jeder Schüler auf seinem Arbeitsrechner durchführen!

Durch die Wahl von *schule* wird das Schlüsselpaar 
...
Die Berechtigungen der Datei sollten auf 644 gestellt sein:

```bash
chmod 644 /home/git/.ssh/authorized_keys
```

Das kopieren geht mit Putty, wenn man auf Windows den Key in die Zwischenablage
kopiert (\strg{c}) und in putty die Datei mit *nano* geöffnet hat. 
Ein Rechtsklick fügt dann dort den Inhalt der Zwischenablage ein. Das hat  
zumindest bei meiner Windows-Version funktioniert.

Im Prinzip kann ab jetzt kennwortfrei mit \git gearbeitet werden.
...
auf diese Weise nicht mehr möglich. Für \git  spielt dies
allerdings keine Rolle, da bei \git-Befehlen das Terminal nicht gestartet 
...
ist und sie in die Datei \datei{authorized_keys} einfügt.
\sampleend
